...
- 当前仓库的核心能力包括：自动按键、挖种子、自动回点、系统关机管理，以及与这些能力相关的窗口激活、识图、颜色检测和键鼠模拟。
...
- `tlbb/src/dig_seed.py`：挖种子相关业务流程。
...
- `keyboard_simulator.py`、`auto_return.py`、`dig_seed.py` 的运行可能干扰用户当前桌面，不要在未经说明的情况下直接长时间运行自动化流程。
...
- 优先做静态验证和低风险验证，例如：
  ...
  - 打包配置检查
  - 资源路径检查
- 如果改动涉及识图、窗口激活、键鼠操作、回点流程、挖种子流程，完整验证通常需要真实游戏窗口和桌面环境。
